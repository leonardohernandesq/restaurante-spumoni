...
const Finalizar = () => {
    ...
    const loja = 'Estrada da Giesteira 65/67, Arruda dos Vinhos, 2630-241, Portugal'
    ...
    const { endereco, showModal, setShowModal, abrirModalEndereco, addressError, setAddressError, errorEndereco, modalEndereco,
        ...
        setCepValue, modalReferencia, handleInputChange, handleCepChange, handleEndereco, preencherEnderecoAutomaticamente, distanciaCliente
    } = useEndereco();

    const { subtotal, taxaEntrega, valorFinal } = useResumoPedido(produtos, distanciaCliente, delivery);
    ...
    const handleFinish = async () => {
        ...
        if (distanciaCliente !== null && distanciaCliente > 10) {
            toast.error('Desculpe, no momento só entregamos em até 10 km de distância.')
            return
        }
    ...
    return (
        <Container styleRow="bg-zinc-100">
            ...
            <main className="relative py-5 pb-28 gap-4 flex flex-col flex-1 min-h-screen">
                ...
                <div>
                    <label htmlFor='nome_cliente'>Nome: <RequiredAsteristic /></label>
                    <input value={name} onChange={(e) => setName(e.target.value)} name="nome-cliente" id="nome-cliente" type="text" placeholder="Digite o seu nome" className="w-full p-2 border border-zinc-400 rounded-md" required />
                </div>

                <div>
                    <label htmlFor='whatsapp'>Whatsapp: <RequiredAsteristic /></label>
                    <input value={phone} onChange={(e) => setPhone(e.target.value)} name="whatsapp" id="whatsapp" type="text" placeholder="Digite o seu telefone" className="w-full p-2 border border-zinc-400 rounded-md" required />
                </div>
                ...
                <BookingInputs entrega={entrega} setEntrega={setEntrega} delivery={delivery} bookingDate={bookingDate} handleChangeBookingDate={handleChangeBookingDate} />

                <PaymentCheckout payment={payment} setPayment={setPayment} troco={troco} setTroco={setTroco} />
