      ...
      <section className="mt-6">
        {categories.map((item: ICategory) => (
          <div
            id={item.slug}
            key={item.id}
            className="py-6 scroll-mt-20 border-b border-zinc-300 flex"
          >
            <div className="w-full">
              <div className="flex items-center gap-2 mb-2">
                <div
                  title="Número de produtos relacionados"
                  className="flex h-6 w-6 items-center justify-center text-white bg-green-principal-700 text-sm"
                >
                  {item.total_produtos}
                </div>
                <h2 className="font-bold text-xl text-green-principal-700">
                  {item.nome}
                </h2>
              </div>
              <p>{item.descricao}</p>
