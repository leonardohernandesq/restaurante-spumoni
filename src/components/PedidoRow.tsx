...
export const PedidoRow = ({ pedido }: IPedidosData) => {
  ...
  useEffect(() => {
    if (pedido.id) {
      router.prefetch(`/admin/pedido/${pedido.id}`);
    }
  }, [pedido.id, router]);

  return (
    <button
      onClick={() => pedido.id && handleGoToPedido(String(pedido.id))}
      className="cursor-pointer w-full text-left bg-zinc-100 border border-zinc-200 gap-3 my-2 py-2.5 px-3.5 rounded-md flex justify-between items-center relative"
    >
